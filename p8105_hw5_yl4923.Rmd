...
```
**Comments:** In the beginning of the experiment, the longitudinal data observed both in the control group and the experimental group are quite similar, which is around 1.5. Following along, `exp` arm is generally higher than observations in `con` arm. A t-test with _alternative hypothesis_: true difference in means between group exp and group con is greater than 0 can support the difference between arms. Since **p.value = `r t.test(observation ~ arm, data = exp_df, alternative = "greater")$p.value`** is smaller than a significant level at 5%, we can conclude that the experimental arm and control arm are significantly different from each other. So the experimental arm may have a positive impact on the longitudinal data.